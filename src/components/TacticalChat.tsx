...
export default function TacticalChat() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: data.reply || 'Connection lost. Retrying...' }]);
    } catch (e) {
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 w-80 md:w-96 h-[500px] bg-card/95 backdrop-blur-xl border border-primary/50 rounded-lg shadow-2xl z-[100] flex flex-col overflow-hidden"
          >
            {/* Header */}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 font-mono text-xs" ref={scrollRef}>
              ...
              {loading && (
                <div className="flex justify-start">
                  <div className="bg-secondary/10 text-secondary p-2 rounded border border-secondary/30 animate-pulse">
                    ANALYZING...
                  </div>
                </div>
              )}
            ...
            <div className="p-3 border-t border-primary/30 bg-background/50 flex gap-2">
              <input
                ...
                className="flex-1 bg-background border border-primary/30 rounded px-2 py-1 text-xs font-mono focus:outline-none focus:border-primary text-foreground"
              />
              <button 
                onClick={handleSend}
                disabled={loading}
                className="p-2 bg-primary/20 hover:bg-primary/40 text-primary rounded border border-primary/50 transition-colors"
              >
