...
import { AlertTriangle, Globe, User, Sun, Moon, LogOut, Settings, Key } from 'lucide-react';
  ...
  return (
    <header className="h-16 bg-background/90 backdrop-blur-md border-b border-border flex items-center justify-between px-6 z-50">
      ...
      <div className="flex items-center gap-4">
        {mounted && (
          <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} className="p-2 rounded hover:bg-primary/10 text-muted-foreground hover:text-primary transition-colors">
        ...
        <div className="relative" ref={menuRef}>
          <button onClick={() => setMenuOpen(!menuOpen)} className="w-9 h-9 rounded-full bg-card border border-primary/30 flex items-center justify-center hover:bg-primary/20 transition-colors overflow-hidden">
            {user?.photoURL ? (
              <Image src={user.photoURL} alt="User" width={36} height={36} unoptimized className="w-full h-full object-cover" />
            ) : (
          ...
          <AnimatePresence>
            {menuOpen && (
              <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className="absolute right-0 top-full mt-2 w-56 bg-card border border-primary/30 rounded shadow-2xl overflow-hidden backdrop-blur-xl z-[100]">
                ...
                <div className="p-2 border-t border-border bg-background text-[9px] text-center text-muted-foreground font-mono">v2.4.0-STABLE</div>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
